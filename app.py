...
env_loaded=load_dotenv() # take the environment variables from.env
if env_loaded:
...
st.set_page_config(page_title="VidNotes AI")
logging.info("Streamlit app initialized.")
st.header("VidNotes AI")
st.write("An intuitive platform that seamlessly converts your YouTube videos into comprehensive notes")

# Create a Sidebar
sidebar=st.sidebar

# create the text area for video link
video_url=st.text_input("Enter the Youtube Video Link:",key='input')
...
text_area=sidebar.text_area("Write Your text Prompt here :",value=prompt_value,key="text")
...
##v Created an animated button with an emoji
